...
function setupSegmentation() {
    ...
    $('#canvas').mousedown(function(e) {
        ...
        if(point !== false) {
        ...
        } else if(g_currentTargetFrameIdx >= 0) {
            ...
            if(section >= 0) {
                ...
                // Add point at end
                addControlPoint(mouseX, mouseY, g_currentTargetFrameIdx, g_currentLabel, g_shiftKeyPressed);
            }
        }
        redrawSequence();
    });
    ...
    $('#canvas').mousemove(function(e) {
        ...
        if(g_move && g_currentTargetFrameIdx >= 0) {
            cursor = 'move';
            setControlPoint(g_pointToMove, g_labelToMove, mouseX, mouseY);
            redrawSequence();
        } else {
            if(g_currentTargetFrameIdx >= 0) {
                if(g_controlPoints[g_currentTargetFrameIdx][g_currentLabel].length > 0 && isPointOnSpline(mouseX, mouseY) < 0) {
...
function getClosestPoint(x, y) {
    ...
    for(var label_idx = 0; label_idx < g_labelButtons.length; label_idx++) {
        for(var i = 0; i < g_controlPoints[g_currentTargetFrameIdx][label_idx].length; i++) {
...
function insertControlPoint(x, y, label, index) {
    ...
    g_controlPoints[g_currentTargetFrameIdx][g_currentLabel].splice(index+1, 0, controlPoint);
}

function addControlPoint(x, y, target_frame, label, uncertain) {
    var controlPoint = createControlPoint(x, y, label, uncertain);
    g_controlPoints[target_frame][label].push(controlPoint);
}

function getControlPoint(index, label) {
    if(index === -1) {
        index = g_controlPoints[g_currentTargetFrameIdx][label].length-1;
    }
    return g_controlPoints[g_currentTargetFrameIdx][label][index];
}

function setControlPoint(index, label, x, y) {
    g_controlPoints[g_currentTargetFrameIdx][label][index].x = x;
    g_controlPoints[g_currentTargetFrameIdx][label][index].y = y;
}

function isPointOnSpline(pointX, pointY) {
    for(var label = 0; label < g_labelButtons.length; label++) {
        for (var i = 0; i < g_controlPoints[g_currentTargetFrameIdx][label].length; ++i) {
            var a = getControlPoint(max(0, i - 1), label);
            var b = getControlPoint(i, label);
            var c = getControlPoint(min(g_controlPoints[g_currentTargetFrameIdx][label].length - 1, i + 1), label);
            var d = getControlPoint(min(g_controlPoints[g_currentTargetFrameIdx][label].length - 1, i + 2), label);
...
function redraw(){
    ...
    for(var labelIndex = 0; labelIndex < g_labelButtons.length; labelIndex++) {
        for (var i = 0; i < g_controlPoints[g_currentTargetFrameIdx][labelIndex].length; i++) {
            ...
            var c = getControlPoint(min(g_controlPoints[g_currentTargetFrameIdx][labelIndex].length - 1, i + 1), labelIndex);
            var d = getControlPoint(min(g_controlPoints[g_currentTargetFrameIdx][labelIndex].length - 1, i + 2), labelIndex);
        ...
        if(g_controlPoints[g_currentTargetFrameIdx][labelIndex].length > 4) {
                var a = getControlPoint(g_controlPoints[g_currentTargetFrameIdx][labelIndex].length-2, labelIndex);
                var b = getControlPoint(g_controlPoints[g_currentTargetFrameIdx][labelIndex].length-1, labelIndex);
...
function redrawSequence() {
    if(g_targetFrames.includes(g_currentFrameNr)) {
        g_currentTargetFrameIdx = g_targetFrames.findIndex(frame => frame === g_currentFrameNr);
    } else {
        g_currentTargetFrameIdx = -1;
        var index = g_currentFrameNr - g_startFrame;
        g_context.drawImage(g_sequence[index], 0, 0, g_canvasWidth, g_canvasHeight);
    }
    redraw();
